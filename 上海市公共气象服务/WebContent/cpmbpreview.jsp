...
<%
	...
	if (session.getAttribute("root") != null && !session.getAttribute("root").equals("")) {
		...
		contextpath = "产品模板/" + root + "/" + node + "/" + filename;
		System.out.print("contextpath：" + contextpath);
		imgPath = "产品模板/" + root + "/" + node + "/" + filename;
	} else {
		contextpath = "产品模板/" + node + "/" + filename;
		imgPath = "产品模板/" + node + "/" + filename;
	}
	...
	if (filename.endsWith(".docx") || filename.endsWith(".doc")) {
		poCtrl.webOpen(contextpath, OpenModeType.docReadOnly, "");
	}
	if (filename.endsWith(".xlsx") || filename.endsWith(".xls")) {
		poCtrl.webOpen(contextpath, OpenModeType.xlsReadOnly, "");
	}
...
<body>
	...
	%>
	<img alt="" src="<%=imgPath%>" style="width: 100%;">
	<%
		} else {
	%>
