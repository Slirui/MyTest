				...
				<tbody>
					<s:foreach list="${qr.list }" var="t" status="st">
						<tr>
							...
							<td>${t.year}</td>
							<td>${s:formatDate(t.modDay,'yyyy-MM-dd') }</td>
							<td>${s:formatDate(t.meiDay,'yyyy-MM-dd') }</td>
							<td>${t.meiyuLength }</td>
							<td>${t.meiyuAmount }</td>
							<td>
								<button onclick="edit('getModById.shtml?Id=${t.id}')"
		...
		function del(Id) {
			BootstrapDialog.confirm({
	            ...
	            callback: function(result) {
	                if(result) {
	                	location.href = "deleteMod.shtml?Id=" + Id;
			...
			BootstrapDialog.show({
				...
				buttons: [{
	                ...
	                action: function(dialogItself){
	                	var modDay = $("iframe").contents().find("#modDay").val(); 
	                	var meiDay = $("iframe").contents().find("#meiDay").val(); 
	                	var meiyuAmount = $("iframe").contents().find("#meiyuAmount").val(); 
	                	var Id = $("iframe").contents().find("#meiyuId").val();
	                	location.href="updateMod.shtml?modDay="+modDay+"&meiDay="+meiDay+"&meiyuAmount="+meiyuAmount+"&Id="+Id;
	                	dialogItself.close();
	                }
				}]
			});
