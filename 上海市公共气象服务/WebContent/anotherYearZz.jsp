...
<script type="text/javascript">
	$(function() {
		...
		$("#export").click(
				function() {
					...
					location.href = "exportDateMin.shtml?dx="
	...
	function search() {
		...
		parent.pageGetInfo.location.href = "getAnotherYears2.shtml?dx="
				+ encodeURIComponent(dx) + "&beginYear=" + beginYear
				+ "&endYear=" + endYear;
	}
</script>
	...
	<div class="topselect">
		<ul>
			...
			<li>
				<div>
					<p>开始年份：</p>
					<s:select list="${dateYears }" id="beginYear" value="${beginYear }"
						name="beginYear"></s:select>
				</div>
				<div>
					<p>结束年份：</p>
					<s:select list="${dateYears }" id="endYear" value="${endYear }"
						name="endYear"></s:select>
				</div>
	...
	<div class="sltresult">
		<div class="rsttitle">
			...
			<div id="edit" class="daochu" style="${sessionScope.userName == 'admin'?'':'display:none;'}" >
				...
				<p
					style="color: #29ba9c; margin: 0; font-size: 11px; line-height: 16px;">编辑</p>
			</div>
			<div id="add" class="daochu" style="${sessionScope.userName == 'admin'?'':'display:none;'}">
				<img src="images/add.png" />
				<p
					style="color: #29ba9c; margin: 0; font-size: 11px; line-height: 16px;">新增</p>
			</div>
		</div>
